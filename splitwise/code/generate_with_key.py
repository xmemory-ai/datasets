...
def generate_expense_prompts_batched(events, batch_size, progress=None):
    ...
    for start in range(0, n, batch_size):
        ...
        prompt = (
            ...
            f"- FORBIDDEN: relative-time phrasings that depend on context from a previous outing "
            f"(\"the next day\", \"the following day\", \"the next evening\", \"the next morning\", "
            f"\"that evening\", \"that night\", \"the day before\", \"the previous day\", "
            f"\"a week later\", etc.). Each sentence must be self-contained — every date reference "
            f"must resolve to the assigned ISO date on its own, without relying on any earlier "
            f"sentence as an anchor.\n\n"
